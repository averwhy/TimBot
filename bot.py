...
class RoboTim(commands.AutoShardedBot):
    ...
    async def create_ticket(self, channel: discord.TextChannel, from_message: discord.Message, thread: discord.Thread, author: discord.Member, form_response: list = None) -> int:
        ...
        else: await self.pool.execute("INSERT INTO tickets VALUES (DEFAULT, $1, $2, 0, $3, $4, DEFAULT, DEFAULT, NULL, DEFAULT, $5) RETURNING id;", channel.id, from_message.id, author.id, thread.id, author.guild.id)
    ...
    async def get_tickets(self, ticket_id: int = None, thread_id: int = None, closed_by: int = None, closed: bool = None) -> typing.Union[tuple, typing.List[tuple], None]:
        ...
        if ticket_id:
            ...
            if closed:
                ...
                result = await self.pool.fetch("SELECT * FROM tickets WHERE statuscode = 1")
            elif closed is False:
                log.info("getting closed tickets")
                result = await self.pool.fetch("SELECT * FROM tickets WHERE statuscode = 0")
            if result: 
                for r in result: results.append(r)
        elif all([isinstance(item, NoneType) for item in (ticket_id, thread_id, closed_by)]):
